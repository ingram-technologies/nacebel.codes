...
function mapToPublicNacebelCode(code: NacebelCode, includeChildren = false): PublicNacebelCode {
...
async function loadNacebelData(): Promise<void> {
  if (nacebelData && nacebelCodeMap && nacebelIdWithoutDotsMap && nacebelChildrenMap) {
...
export async function getPaginatedNacebelCodes(
  ...
  const data = allCodes.slice(startIndex, endIndex).map((code) => mapToPublicNacebelCode(code))
...
export async function searchNacebelCodes(
  ...
  const results = allCodes.filter((codeEntry) => {
    ...
    return queryTokens.every(
      (token) =>
        ...
        Object.values(codeEntry.searchableTitles).some((title) => title.includes(token)),
    )
  })
  ...
  results.sort((a, b) => {
    ...
    if (aCode.startsWith(searchableQuery) && !bCode.startsWith(searchableQuery)) return -1
    if (bCode.startsWith(searchableQuery) && !aCode.startsWith(searchableQuery)) return 1

    // Prioritize title starts with query (any language)
    if (aTitles.some((t) => t.startsWith(searchableQuery)) && !bTitles.some((t) => t.startsWith(searchableQuery)))
      return -1
    if (bTitles.some((t) => t.startsWith(searchableQuery)) && !aTitles.some((t) => t.startsWith(searchableQuery)))
      return 1

    // Prioritize code contains query
    if (aCode.includes(searchableQuery) && !bCode.includes(searchableQuery)) return -1
    if (bCode.includes(searchableQuery) && !aCode.includes(searchableQuery)) return 1

    // Prioritize title contains query (any language)
    if (aTitles.some((t) => t.includes(searchableQuery)) && !bTitles.some((t) => t.includes(searchableQuery))) return -1
    if (bTitles.some((t) => t.includes(searchableQuery)) && !aTitles.some((t) => t.includes(searchableQuery))) return 1
  ...
  const startIndex = (page - 1) * limit
  const endIndex = startIndex + limit
  const data = results.slice(startIndex, endIndex).map((code) => mapToPublicNacebelCode(code))
  return { data, totalPages, totalItems }
}

export async function getNacebelCodeDetails(idWithoutDots: string): Promise<PublicNacebelCode | null> {
  await loadNacebelData()
